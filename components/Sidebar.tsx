...
import { PlusIcon, TrashIcon, BookIcon, SunIcon, MoonIcon, XMarkIcon } from './Icons';
...
export const Sidebar: React.FC<SidebarProps> = ({ notebooks, activeNotebookId, onSelectNotebook, onAddNotebook, onDeleteNotebook, isDarkMode, onToggleDarkMode, isOpen, onClose }) => {
    return (
        <>
            {/* Overlay for mobile */}
            <div
                className={`fixed inset-0 bg-black/60 z-10 md:hidden transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                onClick={onClose}
                aria-hidden="true"
            ></div>
                ...
                <header className="mb-4">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                             ...
                             <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 md:hidden transition-colors" aria-label="Close sidebar">
